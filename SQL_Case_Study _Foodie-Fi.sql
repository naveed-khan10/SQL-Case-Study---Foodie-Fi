...
VALUES
  ...
  SELECT
    s.customer_id,   -- Selecting the customer_id column from the subscriptions table
    p.plan_name,     -- Selecting the plan_name column from the plans table
    s.start_date     -- Selecting the start_date column from the subscriptions table
FROM 
    subscriptions s  -- Alias subscriptions table as "s"
JOIN 
    plans p          -- Alias plans table as "p"
ON 
    s.plan_id = p.plan_id;  -- Joining subscriptions and plans tables based on plan_id column
...
ROUND((count(DISTINCT customer_id)/(SELECT COUNT(DISTINCT customer_id) FROM subscriptions)) * 100, 1) 
AS percentage
FROM subscriptions 
 WHERE plan_id=4;
...
FROM My_CTE mc 
INNER JOIN plans as P ON mc.plan_id = P.plan_id
WHERE rwnmbr = 1
GROUP BY plan_name;
...
SELECT 
 count(*) AS count_annual_plan_2020
FROM subscriptions
